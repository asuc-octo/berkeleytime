...
or playlist Course object) and return True if the course satisfies the
constraint.
"""
...
def course_not_in(courses):
    """Constraint returns True if "{abbreviation} {course_number}" not in the given list."""
...
def course_integer_in(course_integers):
    """Constrain the numeric portion of course.course_number is in course_integers."""
    def _satisfies(course):
...
def course_integer_not_in(course_integers):
    """Constrain the numeric portion of course.course_number is not in course_integers."""
    def _satisfies(course):
        # Extract all digits from course_number, there should only be 1
        integers = re.findall(r'\d+', course.course_number)
...
def not_in_definitions(definitions):
    """Return True only if ALL of the definitions are NOT satisfied by the course."""
    def satisfies(course):
...
def not_in_abbreviations(abbreviations):
    """Constraint returns True only if the course is not in any of the given department abbreviations."""
    def satisfies(course):
