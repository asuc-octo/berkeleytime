...
const fileToMongo = async (file: string) => {
  ...
  lines.forEach((line) => {
    try {
      ...
      const obj: any = {
        CourseControlNbr: parseInt(
          getOrDefault(values, headerMap, "Course Control Nbr") ?? ""
        ),
        CourseNumber: getOrDefault(values, headerMap, "Course Number"),
        CourseSubjectShortNm: getOrDefault(
          values,
          headerMap,
          "Course Subject Short Nm"
        ),
        CourseTitleName: getOrDefault(values, headerMap, "Course Title Nm"),
        EnrollmentCnt: parseInt(
          getOrDefault(values, headerMap, "Enrollment Cnt") ?? ""
        ),
        ...
        InstructorName: (
          getOrDefault(values, headerMap, "Instructor Name") ?? ""
        ).split("; "),
      ...
      Object.keys(obj).forEach(
        (k) =>
          obj[k] == null ||
          obj[k] == "" ||
          (Number.isNaN(obj[k]) && delete obj[k])
      );
      objects.push(obj);
    } catch (err) {
  ...
  console.log(
    `Processing for ${file} complete, ${objects.length} documents to be added`
  );
