...
export function formatClass(cls: ClassType, term: Term) {
    return {
...
export function formatSection(section: SectionType, term: Term) {
    ...
    const instructors = meeting.assignedInstructors?.map(i => {
        // Primary name has precedence over preferred name
        let nameInfo = i.instructor?.names.find(n => n.type?.code === "PRI")
        if (nameInfo == undefined) {
            i.instructor?.names.find(n => n.type?.code === "PRF")
        }

        return nameInfo?.formattedName as string
    });
