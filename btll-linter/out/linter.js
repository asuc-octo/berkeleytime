...
exports.parseBtLLLine = parseBtLLLine;
exports.lintBtLL = lintBtLL;
// ── Type registry ──────────────────────────────────────────────────────────────
const PRIMITIVE_TYPES = new Set([
    "boolean", "number", "string", "raw",
]);
const OBJECT_TYPES = new Set([
    "Plan", "Column", "Course", "Label",
    "Requirement", "BooleanRequirement", "NCoursesRequirement",
    "CourseListRequirement", "AndRequirement", "OrRequirement", "NumberRequirement",
]);
 ...
 */
function findMatchingClose(str, openIdx, open, close) {
    let depth = 0;
...
function isValidType(type) {
    ...
    if (!type)
        return false;
    if (PRIMITIVE_TYPES.has(type) || OBJECT_TYPES.has(type))
        return true;
 ...
 */
function checkBracketBalance(expr) {
    const stack = [];
    let inSingle = false;
    let inDouble = false;
    const matching = { ")": "(", "]": "[", "}": "{" };
    ...
    if (inSingle)
        return "Unclosed single-quote string literal";
    if (inDouble)
        return "Unclosed double-quote string literal";
    if (stack.length > 0)
 ...
 */
function parseBtLLLine(lines, startIdx) {
    const parts = [];
    let currentPart = "";
    let angleDepth = 0;
    let parenDepth = 0;
    ...
    while (currentIndex < lines.length && parts.length < 3) {
        ...
        if (parts.length === 3)
            break;
        const stillOpen = angleDepth !== 0 || parenDepth !== 0 || squareDepth !== 0 || curlyDepth !== 0;
        if (stillOpen) {
            if (currentPart.trim())
                currentPart += "\n";
            currentIndex++;
 ...
 */
function lintBtLL(code, lineBase = 0, knownNames = new Set(BUILTIN_FUNCTIONS)) {
    const diagnostics = [];
    const lines = code.split("\n");
    // Track names defined in this scope so we can warn on use of unknown functions.
    const localNames = new Set(knownNames);
    ...
    function err(lineOffset, colStart, colEnd, message) {
        diagnostics.push({
    ...
    function warn(lineOffset, colStart, colEnd, message) {
        diagnostics.push({
            lineOffset: lineBase + lineOffset,
            colStart,
            colEnd,
    ...
    while (i < lines.length) {
        ...
        if (!isValidType(type)) {
            ...
            err(lineNum, col, col + type.split("\n")[0].length, `Unknown or invalid type '${type.split("\n")[0]}'`);
        }
        // ── 3. Variable name must be present ──
        if (!varname) {
            err(lineNum, type.length + 1, trimmed.length, "Missing variable name after type");
            i = endIdx + 1;
            continue;
        }
        ...
        if (varname !== "return") {
            if (!isValidIdentifier(varname)) {
                ...
                err(lineNum, col, col + varname.length, `Invalid identifier '${varname}'`);
            }
            if (PROTECTED_KEYWORDS.has(varname)) {
                const col = tokenCol(line, varname);
                err(lineNum, col, col + varname.length, `'${varname}' is a protected keyword and cannot be used as a variable name`);
            }
        }
        ...
        if (type.startsWith("Function<")) {
            ...
            if (parenStart === -1) {
                err(lineNum, 0, trimmed.length, "Function definition is missing its parameter list '(...)'");
                i = endIdx + 1;
                continue;
            }
            const parenEnd = findMatchingClose(expr, parenStart, "(", ")");
            if (parenEnd === -1) {
                err(lineNum, 0, trimmed.length, "Function parameter list is missing closing ')'");
                i = endIdx + 1;
                continue;
            }
            ...
            if (paramStr !== "") {
                ...
                for (const param of params) {
                    if (!isValidIdentifier(param)) {
                        ...
                        err(lineNum, col, col + param.length, `Invalid parameter name '${param}'`);
                    }
                }
            }
            // Validate arg count matches function type signature
            const typeArgStr = type.slice(type.indexOf("(") + 1, type.lastIndexOf(")")).trim();
            const typeArgCount = typeArgStr === "" ? 0 : splitTopLevel(typeArgStr, ",").length;
            const paramCount = paramStr === "" ? 0 : splitTopLevel(paramStr, ",").length;
            if (typeArgStr !== "" && typeArgCount !== paramCount) {
                err(lineNum, 0, trimmed.length, `Function type declares ${typeArgCount} parameter(s) but definition has ${paramCount}`);
            }
            // Function body: must have { ... }
            const braceStart = expr.indexOf("{", parenEnd);
            if (braceStart === -1) {
                err(lineNum, 0, trimmed.length, "Function definition is missing its body '{ ... }'");
                i = endIdx + 1;
                continue;
            }
            ...
            if (bodyText) {
                ...
                const bodyScope = new Set(localNames);
                if (paramStr !== "") {
            ...
            if (!hasExternalImports) {
                const callMatch = expr.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:<[^>]*>)?\s*\(/);
                if (callMatch) {
                    const callee = callMatch[1];
                    if (!localNames.has(callee) && !BUILTIN_FUNCTIONS.has(callee)) {
                        const col = tokenCol(line, callee);
                        warn(lineNum, col, col + callee.length, `Unknown function or variable '${callee}'`);
                    }
                }
            }
            ...
            if (!hasExternalImports &&
                ...
                warn(lineNum, col, col + expr.length, `'${expr}' is not defined in this scope`);
            }
            ...
            if (exprTrimmed.startsWith("[") && exprTrimmed.endsWith("]")) {
                ...
                if (inner === "" && type.startsWith("List<")) {
                    warn(lineNum, tokenCol(line, "["), tokenCol(line, "]") + 1, "Empty list — consider specifying a type");
                }
            }
            // e) Validate string literal is properly quoted
            if (type === "string") {
                const stripped = exprTrimmed;
                const isDoubleQuoted = stripped.startsWith('"') && stripped.endsWith('"') && stripped.length >= 2;
                const isSingleQuoted = stripped.startsWith("'") && stripped.endsWith("'") && stripped.length >= 2;
                const isVarRef = /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(stripped);
                const isFunctionCall = stripped.endsWith(")");
                if (!isDoubleQuoted && !isSingleQuoted && !isVarRef && !isFunctionCall) {
                    const col = tokenCol(line, stripped.split("\n")[0]);
                    err(lineNum, col, col + stripped.length, "String value must be quoted with \" or '");
                }
            }
            ...
            if (type === "boolean") {
                ...
                if (!isLiteral && !isVarRef && !isFunctionCall) {
                    ...
                    err(lineNum, col, col + stripped.length, "Boolean expression must be 'true', 'false', a variable, or a function call");
                }
            }
            ...
            if (type === "number") {
                ...
                if (!isNumber && !isVarRef && !isFunctionCall) {
                    const col = tokenCol(line, stripped.split("\n")[0]);
                    warn(lineNum, col, col + stripped.length, "Expected a number literal, variable, or function call for number type");
                }
            }
