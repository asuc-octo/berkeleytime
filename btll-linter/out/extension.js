...
function makeDecoration(dark, light, extra) {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerHoverProvider(LANGUAGE_SELECTOR, { provideHover }));
...
function processEditor(editor) {
    ...
    // ── Lint ──────────────────────────────────────────────────────────────────────
    const allVsDiagnostics = [];
    const diagByLine = new Map();
    for (const loc of locations) {
        const btllDiags = (0, linter_1.lintBtLL)(loc.content);
...
// ── Decoration update ──────────────────────────────────────────────────────────
function updateDecorations(editor, locations) {
    const doc = editor.document;
    // Collect ranges for each decoration kind.
    const rangeMap = new Map(Object.keys(DECORATIONS).map((k) => [k, []]));
    for (const loc of locations) {
        ...
        if (markerDocLine < doc.lineCount) {
            ...
            if (markerIdx >= 0) {
                rangeMap.get("marker").push(new vscode.Range(markerDocLine, markerIdx, markerDocLine, markerIdx + 4));
            }
        }
        // Tokenize the BtLL code and map each token back to a document range.
        const tokens = (0, tokenizer_1.tokenizeBtLL)(loc.content);
        for (const tok of tokens) {
            ...
            if (start >= end)
                continue;
            rangeMap.get(tok.kind)?.push(new vscode.Range(docLine, start, docLine, end));
        }
    }
    // Apply (or clear) each decoration type.
    for (const [kind, decType] of Object.entries(DECORATIONS)) {
...
function provideHover(document, position) {
    ...
    for (const loc of cached.locations) {
        ...
        if (position.line === loc.markerLine) {
            ...
            md.appendMarkdown("Berkeleytime Logical Language embedded in this string.\n\n");
            ...
            if (errorCount > 0) {
                md.appendMarkdown(`$(error) **${errorCount} error${errorCount > 1 ? "s" : ""}**   `);
            }
            if (warnCount > 0) {
                md.appendMarkdown(`$(warning) **${warnCount} warning${warnCount > 1 ? "s" : ""}**`);
            }
        ...
        if (btllLine < 0 || btllLine >= lines.length)
            continue;
        const md = new vscode.MarkdownString(undefined, true);
        md.supportThemeIcons = true;
        md.isTrusted = true;
        ...
        const [type, varname, expr] = (0, linter_1.parseBtLLLine)(lines, btllLine);
        if (type || varname) {
...
function toVscodeDiagnostic(document, btllStartLine, diag) {
    const docLine = Math.min(btllStartLine + diag.lineOffset, document.lineCount - 1);
 ...
 */
function detectBtLLMarker(content) {
    ...
    if (newlineAfterMarker === -1)
        return null; // no code after marker
    return {
 ...
 *   `\n// BtLL\n<code>`
 */
function findBtLLInTemplateLiterals(text) {
    const results = [];
    ...
    while (i < text.length) {
        ...
        if (!marker)
            continue;
        const markerLine = offsetToLine(lineStarts, contentStart + marker.btllOffset);
 ...
 */
function findBtLLInPythonStrings(text) {
    const results = [];
    const lineStarts = buildLineStarts(text);
    for (const delim of ['"""', "'''"]) {
        let pos = 0;
        while (pos < text.length) {
            ...
            if (marker) {
                results.push({
                    markerLine: offsetToLine(lineStarts, contentStart + marker.btllOffset),
