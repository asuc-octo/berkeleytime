...
export type TokenKind =
  | "type" // boolean, number, string, List<T>, Function<...>(...)
  | "keyword" // return, main
  | "builtin" // or, and, filter, add, get_attr, …
  | "user-func" // user-defined function calls
  | "string" // "…" / '…'
  | "number" // 42, 3.14
  | "boolean" // true, false
...
export interface BtLLToken {
  /** 0-based line within the BtLL code block (not the document). */
  line: number;
  colStart: number;
  colEnd: number;
  kind: TokenKind;
 ...
 */
export function tokenizeBtLL(code: string): BtLLToken[] {
  const tokens: BtLLToken[] = [];
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    if (varname) {
      const kind: TokenKind = KEYWORD_NAMES.has(varname)
        ? "keyword"
        : "identifier";
      tokens.push({
        line: lineIdx,
      ...
      if (kind === "identifier") userDefinedNames.add(varname);
    }
 ...
 */
function netDepthChange(line: string, col: number): number {
  let depth = 0;
  let inSingle = false;
  let inDouble = false;
 ...
 */
function findTypeEnd(line: string, start: number): number {
  let depth = 0;
  let i = start;
  while (i < line.length) {
    const ch = line[i];
  ...
  while (i < line.length) {
    ...
    if (ch === '"' || ch === "'") {
      ...
      while (j < line.length && line[j] !== ch) {
        if (line[j] === "\\") j++; // skip backslash-escaped character
        j++;
      }
