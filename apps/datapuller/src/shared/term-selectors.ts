...
export const getRecentPastTerms = async () => {
  const pastTerms = await NewTermModel.find({ temporalPosition: "Past" }).lean().sort({ endDate: -1 }).limit(8);

  return pastTerms;
}

export const getActiveTerms = async () => {
  return NewTermModel.find({ temporalPosition: { $in: ["Current", "Future"] } }).lean();
}
