...
import { construct, FUNCTION_MAP, PROTECTED_KEYWORDS, VARIABLE_MAP } from "./language";
import { Data, isGenericType, matchTypes, StringToType, Type } from "./types";

export const run = (code: string, debug: boolean = false) => {
  ...
  for (const line of lines) {
    ...
    const expr_val = evaluate(expr, return_type, debug);
    if (debug) console.log(`EXPR VAL: ${expr_val.data} (${expr_val.type})`);
...
export const evaluate = (expr: string, expected_type: Type, debug: boolean = false): Data<any> => {
  if (debug) console.log(VARIABLE_MAP)
  if (debug) console.log(`EVALUATE: "${expr}" (${expected_type}) ${VARIABLE_MAP.has(expr)}`);
  ...
  if (functionMatch) {
    ...
    const args_val = args.map((arg, i) => evaluate(arg, (genericArgs.length) ? genericArgs[i] : func.args[i], debug));

    if (debug) console.log(`FUNC_EVAL ARGS:`, args_val);
    ...
    if (PROTECTED_KEYWORDS.has(expr)) {
      ...
      if (expr.startsWith("\"") && expr.endsWith("\"")) {
        return construct("string", expr);
      } else if (expr.startsWith("'") && expr.endsWith("'")) {
        return construct("string", expr);
      } else if (Number.isFinite(Number(expr))) {
        return construct("number", expr);
      } else if (expr.startsWith("[") && expr.endsWith("]")) {
        return construct("List<T>", expr);
      }
      return {
        data: expr,
        type: "raw"
