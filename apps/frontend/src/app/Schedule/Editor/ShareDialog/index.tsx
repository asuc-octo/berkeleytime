  ...
  const generateICS = (schedule: any) => {
    ...
    schedule.classes?.forEach((scheduleClass: any) => {
      ...
      selectedSections?.forEach((selectedSection: any) => {
        const section = classInfo.primarySection.sectionId === selectedSection.sectionId 
          ? classInfo.primarySection 
          : classInfo.sections?.find((s: any) => s.sectionId === selectedSection.sectionId);
        ...
        section.meetings.forEach((meeting: any) => {
          ...
          const activeDays = meeting.days
            .map((active: boolean, index: number) => active ? getDayOfWeek(index) : null)
            .filter(Boolean);
          ...
          const instructor = meeting.instructors?.map((i: any) => `${i.givenName} ${i.familyName}`).join(', ') || '';
