...
export default function GradesDrawer({
  ...
  const { data: courseData } = useReadCourseWithInstructor(course.subject, course.number);

  const [ byData, setByData ] = useState<SingleValue<OptionType>>(DEFAULT_BY_OPTION);
  const [selectedInstructor, setSelectedInstructor] = useState<SingleValue<OptionType>>(DEFAULT_SELECTED_INSTRUCTOR);
  const [selectedSemester, setSelectedSemester] = useState<SingleValue<OptionType>>(DEFAULT_SELECTED_SEMESTER);
  ...
  const instructorOptions: OptionType[] = useMemo(() => {
    ...
    const opts = ([...mySet].map((v) => {return {value: v as string, label: v as string}}))
  ...
  const semesterOptions: OptionType[] = useMemo(() => {
    ...
    if (!courseData) return list;
    const filteredClasses = (selectedInstructor?.value === "all" || byData?.value === "semester") ? courseData.classes : courseData.classes.filter((c) => c.primarySection.meetings.find((m) => m.instructors.find((i) => selectedInstructor?.value === `${i.familyName}, ${i.givenName}`)))
    const filteredOptions = filteredClasses.map((c) => { return { value: `${c.semester} ${c.year}`, label: `${c.semester} ${c.year}` } });
    if (filteredOptions.length == 1) {
      if (selectedSemester != filteredOptions[0]) setSelectedSemester(filteredOptions[0]);
  ...
  return <Drawer.Left
    open={open}
    changeOpen={setOpen}
    content={courseData && <div className={styles.content}>
      <div className={styles.header}>
        ...
        <div className={styles.heading}>{courseData.subject} {courseData.number}</div>
      ...
      <div className={styles.body}>
        ...
        <div className={styles.selectCont}>
          {byData?.value === "instructor" ? <Select
            options={instructorOptions}
            value={selectedInstructor}
            onChange={(s) => { setSelectedInstructor(s) }}
          /> : <Select
        ...
        <Button className={styles.button} variant="solid" onClick={() => {
            if (!selectedSemester || !selectedInstructor) return;
            addCourse(course, selectedSemester.value, selectedInstructor.value);
            setOpen(false);
            setSelectedInstructor(DEFAULT_SELECTED_INSTRUCTOR);
            setSelectedSemester(DEFAULT_SELECTED_SEMESTER);
          }}>
