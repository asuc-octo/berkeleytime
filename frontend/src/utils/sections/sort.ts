 ...
 */
function sectionSortComparator(s1: SectionFragment, s2: SectionFragment): number {
	const typeA = s1.kind ? SECTION_TYPE_ORDER[s1.kind] || s1.kind[0].charCodeAt(0) : Infinity;

	const typeB = s2.kind ? SECTION_TYPE_ORDER[s2.kind] || s2.kind[0].charCodeAt(0) : Infinity;
