...
export const isScheduleEmpty = (schedule: Schedule): boolean =>
	!schedule || (schedule.courses.length === 0 && schedule.sections.length === 0);
...
): SchedulerCourseType | null => schedule.courses.find((c) => c.id === section.courseId) || null;

/**
 * Gets the color for a course (based off a section)
 * within a schedule.
 */
export const getColorForSection = (schedule: Schedule, section: SchedulerSectionType): string =>
	getColorForCourse(schedule, section.courseId);
...
export const getUnitsForSchedule = (schedule: Schedule): Units =>
	schedule.courses
		.filter((course) => !!schedule.sections.find((section) => section.courseId === course.id))
		.reduce(
			(sum, course) => (course.units ? addUnits(sum, parseUnits(course.units)) : sum),
			ZERO_UNITS
		);
...
export const removeSection = (schedule: Schedule, sectionId: string) => ({
	...schedule,
	sections: schedule.sections.filter((s) => s.lectureId !== sectionId && s.id !== sectionId)
});
 ...
 */
export const serializeSchedule = (schedule: Schedule, semester: Semester): BackendSchedule => ({
	name: schedule.name,
	...
	selectedSections: schedule.courses
		.map((course) => ({
			...
			secondary: schedule.sections
				.filter((section) => section.courseId === course.id && !!section.lectureId)
...
export function scheduleToICal(schedule: Schedule, semester: Semester): string {
	const SEMESTER_START = new Date(2026, 0, 20); // jan 20th
	const LAST_COURSE_DAY = new Date(2026, 4, 8); // may 8th
	...
	const events = schedule.sections
		...
		.map(([section]) => {
			...
			const firstWeek = section.days.split('').map((day) => setDay(SEMESTER_START, +day as Day));
			const secondWeek = firstWeek.map((day) => addWeeks(day, 1));

			const firstCourseDay = reinterpretDateAsUTC(
				min(firstWeek.concat(secondWeek).filter((instance) => !isBefore(instance, SEMESTER_START)))
			);
 ...
 */
export const formatScheduleError = (error?: Error | null): Error | string | null | undefined =>
	error &&
