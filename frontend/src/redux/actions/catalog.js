...
export function fetchPlaylists(paths) {
  return (dispatch) => {
    if (paths.length >= 4) {
      ...
      return axios
        .get(`/api/catalog/catalog_json/filters/${abbreviation}/${classNum}/`)
        .then((res) => {
          const defaultPlaylists = res.data.default_playlists
            .split(",")
        ...
        .then((res) => {
          ...
          axios
            .get("/api/catalog/filter/", { params: { course_id: courseID } })
            .then((res2) => {
      ...
      return axios.get("/api/catalog/catalog_json/filters/").then((res) => {
        const defaultPlaylists = res.data.default_playlists
          .split(",")
          .map((str) => parseInt(str));
        dispatch(setDefaultPlaylists(new Set(defaultPlaylists)));
