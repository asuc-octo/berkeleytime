...
class LSResource(object):
    ...
    """
    csvs = {
        str(LSPlaylistName.arts_and_literature): 'arts_literature',  # noqa
        str(LSPlaylistName.biological_science): 'biological_science',  # noqa
        str(LSPlaylistName.historical_studies): 'historical_studies',  # noqa
        str(LSPlaylistName.international_studies): 'international_studies',  # noqa
        str(LSPlaylistName.philosophy_and_values): 'philosophy_values',  # noqa
        str(LSPlaylistName.physical_science): 'physical_science',  # noqa
        str(LSPlaylistName.social_and_behavior_sciences): 'social_behavioral_sciences',  # noqa
    }
    breadths = csvs.keys()
    semester_data = (
    ...
    def refresh_current_semester(self):
        ...
        """
        print "Refreshing data from SIS for the current semester: " + CURRENT_SEMESTER + " " + CURRENT_YEAR
    ...
    def _get_sis(self, semester, year):
        ...
        """
        print "Fetching L&S Breadths from SIS for " + semester + " " + year + " (this may take some time)..."
        ...
        while self._process_page(breadth_to_courses, term_id, page_num):
            if page_num % 10 == 0:
                print semester + " " + year + " pages read: " + str(page_num)
            page_num += 1
