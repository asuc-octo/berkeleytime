...
urlpatterns = patterns('',
    ...
    # url(r'^accounts/password/$', 'django.views.defaults.page_not_found'),
                       ...
                       # TODO (Yuxin)
    url(r'^api/exception/$', 'catalog.views.test'),

    # Catalog
    url(r'^api/catalog/$', 'catalog.views.catalog'),
    url(r'^api/catalog_json/$', 'data.views.catalog_context_json'),
    url(r'^api/catalog_json/course/(?P<course_id>\d+)/$', 'catalog.views.course_json'),
    url(r'^api/catalog_json/(?P<abbreviation>.*)/(?P<course_number>.*)/$', 'data.views.catalog_context_json'),
    url(r'^api/catalog_json/course_box/$', 'catalog.views.course_box_json'),
    # legacy for Berkeleytime V1
    url(r'^api/courses/$', 'catalog.views.catalog'),
    url(r'^api/catalog/filter/$', 'catalog.views.filter'),
    url(r'^api/catalog/course/(?P<course_id>\d+)/$', 'catalog.views.course'),
    url(r'^api/catalog/favorite/$', 'catalog.views.favorite'),
    url(r'^api/catalog/course_box/$', 'catalog.views.course_box'),
    url(r'^api/catalog/(?P<abbreviation>.*)/(?P<course_number>.*)/$', 'catalog.views.catalog'),


    # Grades
    url(r'^api/grades/$', 'data.views.grade_render'),
    url(r'^api/grades/course_grades/(?P<course_id>\d+)/$', 'data.views.grade_section_json'),
    url(r'^api/grades/sections/(?P<grade_ids>.*)/$', 'data.views.grade_json'),
